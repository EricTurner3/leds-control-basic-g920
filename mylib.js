...
function findWheel() {
    ...
    */
    ...
    for (let i in devices) {
      ...
      log.info('vendorId');
      log.info(devices[i].vendorId);
      log.info('productId');
      log.info(devices[i].productId);
      log.info('product');
      log.info(devices[i].product);
      log.info('interface');
      log.info(devices[i].interface);
      log.info('path');
      log.info(devices[i].path);

        if (devices[i].vendorId === 1133 &&
            (devices[i].productId === 49766 || devices[i].product === 'G923 Racing Wheel for PlayStation 4 and PC') &&
    ...
    if (devicePath === '') {
        if (options.debug) {
            console.log(color.yellow('findWheel -> Oops, could not find a g923 Wheel. Is it plugged in?\n'))
            process.exit()
        }
    } else if (options.debug) {
        console.log(color.cyan('findWheel -> Found g923 Wheel at ') + devicePath)
    }
